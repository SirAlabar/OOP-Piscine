...
void SFMLRenderer::initializeWorld(SimulationManager& simulation)
{
	...
	worldSize = std::max(worldSize, 56);
...
void SFMLRenderer::buildGraphLayout(const Graph* graph)
{
	...
	for (size_t i = 0; i < nodes.size(); ++i)
	{
		...
		positions[node]  = sf::Vector2f(std::cos(angle) * initialRadius, std::sin(angle) * initialRadius);
	...
	for (int iter = 0; iter < iterations; ++iter)
	{
		...
		for (size_t i = 0; i < nodes.size(); ++i)
		{
			for (size_t j = i + 1; j < nodes.size(); ++j)
			{
				...
				int degreeA = graph->getNeighbors(const_cast<Node*>(a)).size();
				int degreeB = graph->getNeighbors(const_cast<Node*>(b)).size();

				float degreeFactor = 1.0f / std::sqrt((float)(degreeA + degreeB));
		...
		for (const Rail* rail : graph->getRails())
		{
			...
			if (a->getType() != NodeType::CITY && b->getType() != NodeType::CITY)
			{
				desiredLength *= 0.7f;
			}
		...
		for (const Node* node : nodes)
		{
			...
			auto neighbors = graph->getNeighbors(const_cast<Node*>(node));

			if (neighbors.size() == 2)
			{
				sf::Vector2f midpoint =
					(positions[neighbors[0]] + positions[neighbors[1]]) * 0.5f;

				sf::Vector2f delta = midpoint - positions[node];
		...
		for (const Node* node : nodes)
		{
			...
			if (len > maxRadius)
			{
				positions[node] = (positions[node] / len) * maxRadius;
			}
		}
	}
...
void SFMLRenderer::markRailsInWorld(const Graph* graph)
{
	...
	for (const auto& pair : _nodeGridPositions)
	{
		...
		worldPositions[pair.first] = sf::Vector2f(
	...
	for (const Rail* rail : graph->getRails())
	{
		...
		if (_world->isInBounds(from.x, from.y))
		{
			if (from.x != to.x)
			{
				if (sx > 0)
				{
					_world->addRailConnection(from.x, from.y, RailDir::East);
				}
				else
				{
					_world->addRailConnection(from.x, from.y, RailDir::West);
				}
			}
			else if (from.y != to.y)
			{
				if (sy > 0)
				{
					_world->addRailConnection(from.x, from.y, RailDir::South);
				}
				else
				{
					_world->addRailConnection(from.x, from.y, RailDir::North);
				}
			}
		}
		...
		if (_world->isInBounds(to.x, to.y))
		{
			...
			if (from.y != to.y)
			{
				if (sy > 0)
				{
					_world->addRailConnection(to.x, to.y, RailDir::North);
				}
				else
				{
					_world->addRailConnection(to.x, to.y, RailDir::South);
				}
			}
			else if (from.x != to.x)
			{
				if (sx > 0)
				{
					_world->addRailConnection(to.x, to.y, RailDir::West);
				}
				else
				{
					_world->addRailConnection(to.x, to.y, RailDir::East);
				}
			}
