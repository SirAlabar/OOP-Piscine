...
TEST(RailTest, IsOccupiedInitiallyFalse)
{
	Node a("CityA");
	Node b("CityB");
	Rail r(&a, &b, 10.0, 100.0);
	EXPECT_FALSE(r.isOccupied());
	EXPECT_EQ(r.getOccupiedBy(), nullptr);
}

TEST(RailTest, SetAndClearOccupied)
{
	Node a("CityA");
	Node b("CityB");
	Train t;
	...
	r.setOccupiedBy(&t);
	EXPECT_TRUE(r.isOccupied());
	EXPECT_EQ(r.getOccupiedBy(), &t);
